...
!=====================================================================
  ...
  namelist / nml_tstorms / crit_vort, crit_twc, crit_thick, crit_dist, &
                           dist_twc, crit_psl, dist_psl, lat_bound_n, &
                           lat_bound_s, do_spline, do_thickness
...
! xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
  ...
  !if ( vort(i,j) < crit_vort ) CYCLE
  !if( vort(i,j) /= vort_max  ) CYCLE
...
!  if( psl_min < 500.e2 ) ierr_pos = 1
  if( psl_min < crit_psl ) ierr_pos = 1
...
! ------ adjust for cold-core also

!  twc_max = abs(tbar(imm, jmm) - avg)
...
! added thick test without spline routine

else

  call find_minmax(.false., thick(im:ip,jm:jp), imm, jmm, avg)
  imm=i-(nx+1)+imm
  jmm=j-(nx+1)+jmm
  lon_thick = rlon(imm)
  lat_thick = rlat(jmm)

  !thick_max = abs(thick(imm, jmm) - avg)
  thick_max = thick(imm, jmm) - avg

  !PRINT *,'thick_max: ',thick_max
  !PRINT *,'thick: ',thick(imm,jmm)
  !PRINT *,'avg: ',avg
  ...
  exist_thick = (ierr_pos == 0)
  exist_thick = exist_thick .and. (thick_max > crit_thick)
  if( .not.exist_thick ) CYCLE
